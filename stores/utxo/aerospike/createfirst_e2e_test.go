...
// TestSpendAndCreate_CreateFirst_TerminalConflictResolvedInline is the ChiR8 regression:
// a create-first double-spend loser must be resolved to its conflicting terminal state
// inline by the seam, not left as an unresolved tentative record for the sweeper to clean
// up blocks later.
func TestSpendAndCreate_CreateFirst_TerminalConflictResolvedInline(t *testing.T) {
	logger := ulogger.NewErrorTestLogger(t)
	tSettings := test.CreateBaseTestSettings(t)
	tSettings.UtxoStore.UseCreateFirstOrder = true
	...
	// ChiR8: the seam resolves it terminally, inline — no unresolved-record window.
	got := &meta.Data{}
	require.NoError(t, store.GetMeta(ctx, loser.TxIDChainHash(), got))
	require.True(t, got.Conflicting, "the loser must be marked conflicting inline")
	require.False(t, got.Creating, "the loser must be finalized out of creating inline, not left as a sweeper WAL")
}
...
func TestCreateFirstLifecycle(t *testing.T) {
	...
	t.Run("ChiR1: a creating child with an absent input is not falsely blessed", func(t *testing.T) {
