...
impl<'a> VersionParser<'a> {
    ...
    pub fn parse(&self) -> Result<Version> {
        // VENDORED PATCH (teranode): the fourth `.build` component is OPTIONAL,
        // defaulting to 0. The upstream regex required a strict 4-part
        // `major.minor.patch.build`, but the bsv-blockchain aerospike-server
        // fork reports a 3-part build string (e.g. `8.0.0`), which made every
        // node fail validation ("Could not parse node version string").
        ...
        let (major, minor, patch, build) = RE
            ...
            .map(|caps| {
                (
                    ...
                    caps.name("build").map_or("0", |m| m.as_str()),
                )
            })
            .unwrap();
...
mod tests {
    ...
    fn parse() {
        ...
        // VENDORED PATCH (teranode): 3-part build strings are valid, build=0 —
        // the bsv-blockchain aerospike-server fork reports e.g. `8.0.0`.
        assert_eq!(
            Version::new(8, 0, 0, 0),
            VersionParser::new("8.0.0").parse().unwrap()
        );
        assert_eq!(
            Version::new(1, 2, 3, 0),
            VersionParser::new("1.2.3-asdfasdf").parse().unwrap()
        );

        let invalid = vec![".1.2.3.4", "1111..333.14", "1111.a2222.333.14-asdfasdf"];
