...
// RecordValidatedPeerProgress stores locally validated progress for a peer.
func (b *Blockchain) RecordValidatedPeerProgress(_ context.Context, req *blockchain_api.RecordValidatedPeerProgressRequest) (*emptypb.Empty, error) {
	blockHash, err := bytesToBlockHash(req.BlockHash)
	if err != nil {
		return nil, errors.WrapGRPC(errors.NewInvalidArgumentError("validated block hash is invalid", err))
	}
	if err := b.peerRegistry.RecordValidatedPeerProgress(req.PeerId, req.Height, blockHash, req.ChainWork); err != nil {
...
func peerInfoToProto(info *PeerInfo) *blockchain_api.PeerRegistryInfo {
	return &blockchain_api.PeerRegistryInfo{
		...
		Height:                    info.Height,
		AdvertisedHeight:          info.AdvertisedHeight,
		...
		ValidatedHeight:           info.ValidatedHeight,
		ValidatedBlockHash:        blockHashToBytes(info.ValidatedBlockHash),
		ValidatedChainWork:        append([]byte(nil), info.ValidatedChainWork...),
...
func protoToPeerInfo(logger ulogger.Logger, p *blockchain_api.PeerRegistryInfo) *PeerInfo {
	...
	validatedHash, err := bytesToBlockHash(p.ValidatedBlockHash)
	if err != nil && logger != nil {
		logger.Errorf("peer registry: dropping invalid ValidatedBlockHash for peer %s: %v", p.PeerId, err)
	}
	return &PeerInfo{
		ID:                        p.PeerId,
