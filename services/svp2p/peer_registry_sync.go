...
// This file is the svp2p counterpart of services/legacy/peer_registry_sync.go
// (PR 1611): the same reconcile loop against the same registry contract, fed
// by protocol.PeerManager snapshots instead of the legacy internal server's
// peer list. The two services never run together (the daemon refuses that
// combination), so at most one of the two loops exists in a process.

// legacyPeerIDPrefix namespaces a wire-protocol peer inside the shared peer
// registry. A libp2p ID never carries this prefix, so a wire entry is
// self-identifying in logs, filters and the dashboard. It is deliberately the
// SAME prefix the legacy service uses: the registry key is the peer's address,
// so an operator who cuts over from legacy to svp2p keeps one entry per peer
// instead of growing a stale twin under a new prefix.
...
// registryPeerSnapshot is the subset of legacy peer state the registry needs. The
// reconcile loop compares consecutive snapshots, so an idle peer costs no RPC.
type registryPeerSnapshot struct {
	id            string
	addr          string
	userAgent     string
	height        uint32
...
func (s registryPeerSnapshot) registrationEqual(other registryPeerSnapshot) bool {
...
// peerRegistrySync mirrors connected wire-protocol peers into the centralized
// peer registry, so the dashboard can show them beside libp2p peers. It is a
...
type peerRegistrySync struct {
	...
	snapshot   func() []registryPeerSnapshot
	lastSeen   map[string]registryPeerSnapshot
...
func newPeerRegistrySync(logger ulogger.Logger, tSettings *settings.Settings,
	registry blockchain.PeerRegistryClientI, snapshot func() []registryPeerSnapshot) *peerRegistrySync {
	...
	return &peerRegistrySync{
		...
		snapshot:   snapshot,
		lastSeen:   make(map[string]registryPeerSnapshot),
	}
}
...
func (p *peerRegistrySync) run(ctx context.Context) {
	...
	p.logger.Infof("[Svp2pPeerRegistry] started, interval %s", p.interval)

	// Reconcile once up front so a node with live wire peers is not shown as
	// empty for a whole interval. If the peer manager is not running yet,
	...
	for {
		select {
		case <-ctx.Done():
			p.logger.Infof("[Svp2pPeerRegistry] stopped")
			return
		case <-ticker.C:
			p.reconcile(ctx)
		}
...
// reconcile pushes one snapshot of connected wire peers into the registry.
func (p *peerRegistrySync) reconcile(ctx context.Context) {
	peers := p.snapshot()
	if peers == nil {
		// A nil snapshot means the peer manager could not answer (it is not
		// running yet). It does NOT mean every
		// peer went away, so leave lastSeen untouched and retry next tick.
		p.logger.Debugf("[Svp2pPeerRegistry] no peer snapshot available this tick")
		return
	}

	current := make(map[string]registryPeerSnapshot, len(peers))
	...
	for _, snap := range peers {
		...
		if !known || !snap.registrationEqual(previous) {
			...
			if err != nil {
				p.logger.Warnf("[Svp2pPeerRegistry] RegisterPeer %s failed: %v", snap.id, err)
				continue
			}
		}
		...
		if !known {
			...
			if err != nil {
				p.logger.Warnf("[Svp2pPeerRegistry] connect %s failed: %v", snap.id, err)
				continue
			}
		}
		...
		if !known {
			...
			switch {
			case err != nil:
				p.logger.Warnf("[Svp2pPeerRegistry] byte baseline %s failed: %v", snap.id, err)
			case found:
		...
		if sentDelta > 0 || recvDelta > 0 {
			...
			if err != nil {
				p.logger.Warnf("[Svp2pPeerRegistry] metrics %s failed: %v", snap.id, err)
		...
		if snap.lastRecv.After(previous.lastRecv) {
			...
			if err != nil {
				p.logger.Warnf("[Svp2pPeerRegistry] last message %s failed: %v", snap.id, err)
	...
	for id := range p.lastSeen {
		...
		if err != nil {
			p.logger.Warnf("[Svp2pPeerRegistry] disconnect %s failed: %v", id, err)
			continue
		}
...
func (p *peerRegistrySync) adoptExistingEntries(ctx context.Context, current map[string]registryPeerSnapshot) {
	...
	if err != nil {
		...
		p.logger.Warnf("[Svp2pPeerRegistry] startup adoption failed: %v", err)
		return
	}
	...
	for _, info := range peers {
		...
		if stateErr != nil {
			p.logger.Warnf("[Svp2pPeerRegistry] clearing stale %s failed: %v", info.ID, stateErr)
			continue
		}

		p.logger.Infof("[Svp2pPeerRegistry] cleared stale connected entry %s", info.ID)
	}
...
// means the peer's connection object was replaced — transport counters only ever
...
// registrySnapshotFrom maps one protocol peer snapshot onto the registry sync
// view. It reports false for a peer that must not produce a registry entry:
// one whose address cannot be split into host and port.
//
// Two LegacyPeerInfo fields stay zero on this transport: PingMicros (svp2p
// keeps the SVNode ping cadence but does not record the round trip) and
// TimeOffsetSecs (the version message's clock offset is not retained). The
// dashboard shows 0 for both until the protocol package tracks them.
func registrySnapshotFrom(snap protocol.PeerSnapshot) (registryPeerSnapshot, bool) {
	if _, _, err := net.SplitHostPort(snap.Addr); err != nil {
		return registryPeerSnapshot{}, false
	}

	var height uint32
	if snap.StartingHeight > 0 {
		height = uint32(snap.StartingHeight)
	}

	return registryPeerSnapshot{
		id:            legacyRegistryID(snap.Addr),
		addr:          snap.Addr,
		userAgent:     snap.UserAgent,
		height:        height,
		bytesSent:     snap.BytesSent,
		bytesReceived: snap.BytesReceived,
		lastRecv:      snap.LastRecv,
		legacy: blockchain.LegacyPeerInfo{
			Inbound:         snap.Inbound,
			ProtocolVersion: snap.ProtocolVersion,
			ServiceFlags:    uint64(snap.Services),
			StartingHeight:  snap.StartingHeight,
			IsSyncPeer:      snap.SyncStarted,
			TimeConnected:   snap.ConnectedAt,
		},
	}, true
}

// registryPeerSnapshots adapts the peer manager's snapshots to the registry
// sync view. A nil return means the manager is not running yet, and the caller
// must not read that as "no peers connected". An empty slice does mean "no
// peers".
func (s *Server) registryPeerSnapshots() []registryPeerSnapshot {
	if s.manager == nil {
		return nil
	}

	snaps := s.manager.Snapshots()
	out := make([]registryPeerSnapshot, 0, len(snaps))

	for _, snap := range snaps {
		mapped, ok := registrySnapshotFrom(snap)
		if !ok {
			continue
		}
