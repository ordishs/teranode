...
func TestSyncManager_HandleBlockDirect(t *testing.T) {
	...
	err = sm.HandleBlockDirect(t.Context(), "", *blockHash, msgBlock)
	require.NoError(t, err)
}
...
func TestSyncManager_createTxMap(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			block, err := ReadBlockFromFile(tc.blockFilePath)
			require.NoError(t, err)
...
func TestWireTxToGoBtTx(t *testing.T) {
	block, err := ReadBlockFromFile("testdata/000000000000000009631dd3dd7357675d8a1f8925be5e7851c68255531ac5fb.bin")
	require.NoError(t, err)
...
func TestSyncManager_createSubtrees_MultiSubtreeDistribution(t *testing.T) {
	...
	sm := &svp2pBridge{logger: ulogger.TestLogger{}}
...
func Benchmark_createSubtrees(b *testing.B) {
	block, err := ReadBlockFromFile("testdata/00000000000000000488eecd93d6f3767b1ba38668200a6a5349af2e0d4fad3f.bin")
	require.NoError(b, err)
...
func TestSyncManager_quickValidationAllowed(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sm := &svp2pBridge{chainParams: tt.chainParams}
...
// each validator error class to the correct prometheusSvp2pBridgePrewarmErrors label,
...
func TestClassifyAndCountPrewarmError(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			counter := prometheusSvp2pBridgePrewarmErrors.WithLabelValues(tt.label)
