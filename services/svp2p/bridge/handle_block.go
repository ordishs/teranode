...
func (sm *svp2pBridge) HandleBlockDirect(ctx context.Context, peerAddr string, blockHash chainhash.Hash, msgBlock *wire.MsgBlock) (err error) {
	...
	// netsync's sync-peer lastBlockTime refresh (guarding the minutes-long
	// validation below from being mistaken for a stall) does not relocate
	// here: syncPeerState is a netsync-only construct, and svp2pBridge has no
	// peer-liveness tracking of its own. That responsibility now belongs to
	// the transport/protocol layer — the peer idle timer is reset on
	// BlockStream arrival and close (see the svp2p Phase 2 plan's Task 7/11
	// liveness carry-notes).
	...
	ctx, _, deferFn := tracing.Tracer("svp2pBridge").Start(ctx, "HandleBlockDirect",
	...
	// netsync's post-ProcessBlock orphan-transaction reprocessing (retrying
	// orphans against the newly-accepted block, then announcing any that
	// clear via PeerNotifier) does not relocate here: the orphan-tx pool and
	// tx relay/announcement land in Phase 3 together with tx ingestion, per
	// the svp2p Phase 2 plan's Decision 1. svp2pBridge carries none of the
	// orphan-pool or PeerNotifier dependencies this would require.
...
func (sm *svp2pBridge) waitForPreviousBlockMined(ctx context.Context, prevBlockHash *chainhash.Hash, blockHeight uint32) error {
...
func (sm *svp2pBridge) ProcessBlock(ctx context.Context, teranodeBlock *model.Block) (err error) {
	ctx, _, deferFn := tracing.Tracer("svp2pBridge").Start(ctx, "svp2pBridge:processBlock",
		tracing.WithLogMessage(
			sm.logger,
			"[svp2pBridge:processBlock][%s %d] processing block",
			teranodeBlock.Hash().String(),
			teranodeBlock.Height,
		),
		tracing.WithHistogram(prometheusSvp2pBridgeProcessBlock),
	)
	defer func() {
		deferFn(err)
	}()
	...
	if err = sm.blockValidation.ProcessBlock(ctx, teranodeBlock, teranodeBlock.Height, "", "legacy", teranodeBlock.ID); err != nil {
		if errors.Is(err, errors.ErrBlockExists) {
			sm.logger.Infof("[svp2pBridge:processBlock][%s %d] block already exists", teranodeBlock.Hash().String(), teranodeBlock.Height)
			return nil
		}
...
func (sm *svp2pBridge) quickValidationAllowed(blockHeight uint32) bool {
...
//
// Boundary and eligibility live in model.BelowCheckpoint / model.OutpointOnlyEligible — one definition for every path.
func (sm *svp2pBridge) legacyOutpointOnly(height uint32) bool {
	if sm.settings == nil {
		return false
	}
...
// byte-identical.
func (sm *svp2pBridge) legacyUnified(height uint32) bool {
...
func (sm *svp2pBridge) legacyFailClosed(height uint32) bool {
	return sm.settings != nil &&
...
// per block.
func (sm *svp2pBridge) needsParentMinedWait(height uint32) bool {
	return height > 1 && !sm.legacyOutpointOnly(height)
}

func (sm *svp2pBridge) checkSubtreeFromBlock(ctx context.Context, bi blockIdent, subtree *subtreepkg.Subtree) error {
	ctx, _, deferFn := tracing.Tracer("svp2pBridge").Start(ctx, "checkSubtreeFromBlock",
...
func (sm *svp2pBridge) writeSubtree(ctx context.Context, bi blockIdent, subtree *subtreepkg.Subtree,
	subtreeData *subtreepkg.Data, subtreeMetaData *subtreepkg.Meta, quickValidationMode bool) error {
	ctx, _, deferFn := tracing.Tracer("svp2pBridge").Start(ctx, "writeSubtree",
...
func (sm *svp2pBridge) ValidateTransactionsLegacyMode(ctx context.Context, txMap *txmap.SyncedMap[chainhash.Hash, *TxMapWrapper],
	bi blockIdent, blockID uint32) (err error) {
	ctx, _, deferFn := tracing.Tracer("svp2pBridge").Start(ctx, "validateTransactionsLegacyMode",
		tracing.WithHistogram(prometheusSvp2pBridgeValidateTransactionsLegacyMode),
...
// svp2pBridge pipeline.
func (sm *svp2pBridge) candidateFinalityTimesForBlock(ctx context.Context, bi blockIdent) (candidateBlockTime uint32, candidateParentMedianTime uint32, err error) {
...
func (sm *svp2pBridge) candidateParentMedianTimeForBlock(ctx context.Context, parentHash *chainhash.Hash) (uint32, error) {
	if parentHash == nil {
...
func (sm *svp2pBridge) walkParentChain(ctx context.Context, startHash *chainhash.Hash, depth uint64) ([]*model.BlockHeader, error) {
...
// block is valid.
func (sm *svp2pBridge) createUtxos(ctx context.Context, txMap *txmap.SyncedMap[chainhash.Hash, *TxMapWrapper], bi blockIdent, blockID uint32, outpointOnly bool) (err error) {
	_, _, deferFn := tracing.Tracer("svp2pBridge").Start(ctx, "createUtxos",
		tracing.WithLogMessage(sm.logger, "[createUtxos] called for block %s / height %d", bi.hash, bi.height),
		tracing.WithHistogram(prometheusSvp2pBridgeCreateUtxos),
	)
...
func (sm *svp2pBridge) reuseBlockIDFromUTXO(ctx context.Context, bi blockIdent, txOrder []chainhash.Hash) (uint32, bool) {
...
func (sm *svp2pBridge) PreValidateTransactions(ctx context.Context, txMap *txmap.SyncedMap[chainhash.Hash, *TxMapWrapper],
	...
	_, _, deferFn := tracing.Tracer("svp2pBridge").Start(ctx, "PreValidateTransactions",
		tracing.WithLogMessage(sm.logger, "[PreValidateTransactions] called for block %s / height %d", blockHash, blockHeight),
		tracing.WithHistogram(prometheusSvp2pBridgePreValidateTransactions),
	)
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		for _, txHash := range pendingTxHashes {
			...
			g.Go(func() (err error) {
				timeStart := time.Now()
				defer func() {
					prometheusSvp2pBridgeBlockTxValidate.Observe(float64(time.Since(timeStart).Microseconds()) / 1_000_000)
				}()
...
// (validateTransactions) into the prometheusSvp2pBridgePrewarmErrors counter
...
func classifyAndCountPrewarmError(logger ulogger.Logger, err error) {
	...
	case errors.Is(err, errors.ErrTxInvalid):
		prometheusSvp2pBridgePrewarmErrors.WithLabelValues("tx_invalid").Inc()
		logger.Errorf("[validateTransactions][prewarm] critical: tx invalid: %v", err)
	case errors.Is(err, errors.ErrServiceError):
		prometheusSvp2pBridgePrewarmErrors.WithLabelValues("service").Inc()
	...
	case errors.Is(err, errors.ErrTxConflicting), errors.Is(err, errors.ErrTxExists):
		prometheusSvp2pBridgePrewarmErrors.WithLabelValues("policy").Inc()
		logger.Debugf("[validateTransactions][prewarm] expected: %v", err)
	case errors.Is(err, errors.ErrProcessing):
		prometheusSvp2pBridgePrewarmErrors.WithLabelValues("processing").Inc()
		logger.Warnf("[validateTransactions][prewarm] processing error: %v", err)
	default:
		prometheusSvp2pBridgePrewarmErrors.WithLabelValues("other").Inc()
...
func (sm *svp2pBridge) validateTransactions(ctx context.Context, maxLevel uint32, blockTxsPerLevel map[uint32][]*bt.Tx, bi blockIdent) (err error) {
	_, _, deferFn := tracing.Tracer("svp2pBridge").Start(ctx, "validateTransactions",
		tracing.WithLogMessage(sm.logger, "[validateTransactions] called for block %s / height %d", bi.hash, bi.height),
		tracing.WithHistogram(prometheusSvp2pBridgeValidateTransactions),
	)
	...
	for i := uint32(0); i <= maxLevel; i++ {
		_, _, deferLevelFn := tracing.Tracer("svp2pBridge").Start(ctx, fmt.Sprintf("validateTransactions:level:%d", i))
...
func (sm *svp2pBridge) extendTransactions(ctx context.Context, bi blockIdent, txOrder []chainhash.Hash, txMap *txmap.SyncedMap[chainhash.Hash, *TxMapWrapper], outpointOnly bool) (err error) {
	_, _, deferFn := tracing.Tracer("svp2pBridge").Start(ctx, "extendTransactions",
		tracing.WithLogMessage(sm.logger, "[extendTransactions] called for block %s / height %d", bi.hash, bi.height),
		tracing.WithHistogram(prometheusSvp2pBridgeExtendTransactions),
	)
...
func (sm *svp2pBridge) extendFromTxMap(ctx context.Context, tx *bt.Tx, txMap *txmap.SyncedMap[chainhash.Hash, *TxMapWrapper]) error {
	defer func() {
		prometheusSvp2pBridgeBlockTxSize.Observe(float64(tx.Size()))
		prometheusSvp2pBridgeBlockTxNrInputs.Observe(float64(len(tx.Inputs)))
		prometheusSvp2pBridgeBlockTxNrOutputs.Observe(float64(len(tx.Outputs)))
...
func (sm *svp2pBridge) extendPerTxFallback(ctx context.Context, txs []*bt.Tx) error {
	for _, tx := range txs {
...
func (sm *svp2pBridge) createSubtrees(ctx context.Context, bi blockIdent, txOrder []chainhash.Hash, txMap *txmap.SyncedMap[chainhash.Hash, *TxMapWrapper],
	subtreeSlices []*subtreepkg.Subtree, subtreeDatas []*subtreepkg.Data, subtreeMetas []*subtreepkg.Meta, outpointOnly bool) (err error) {
	_, _, deferFn := tracing.Tracer("svp2pBridge").Start(ctx, "createSubtrees",
