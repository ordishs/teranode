...
func testSnapshot(addr string, bytesRecv uint64, lastRecv time.Time) registryPeerSnapshot {
...
func TestReconcile_RegistersNewPeer(t *testing.T) {
	...
	sync := newPeerRegistrySync(ulogger.TestLogger{}, testSyncSettings(), counting,
		func() []registryPeerSnapshot { return []registryPeerSnapshot{snap} })
...
func TestReconcile_VanishedPeerMarkedDisconnectedOnce(t *testing.T) {
	...
	sync := newPeerRegistrySync(ulogger.TestLogger{}, testSyncSettings(), counting,
		func() []registryPeerSnapshot {
			if present {
				return []registryPeerSnapshot{snap}
			}
...
func TestReconcile_ByteCountersAreDeltas(t *testing.T) {
	...
	sync := newPeerRegistrySync(ulogger.TestLogger{}, testSyncSettings(), counting,
		func() []registryPeerSnapshot { return []registryPeerSnapshot{current} })
...
func TestReconcile_DefaultRPCTimeoutIsSet(t *testing.T) {
	...
	sync := newPeerRegistrySync(ulogger.TestLogger{}, testSyncSettings(),
		blockchain.NewLocalPeerRegistryClient(reg), func() []registryPeerSnapshot { return nil })
