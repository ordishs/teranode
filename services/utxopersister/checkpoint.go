...
// checkpoint over (height, blockHash, setHash) with priv, persists the signed
// checkpoint as a sidecar blob, and returns it.
func BuildSignedCheckpoint(ctx context.Context, store blob.Store, blockHash chainhash.Hash, height uint32, priv *bec.PrivateKey) (*seedcheckpoint.SignedCheckpoint, error) {
