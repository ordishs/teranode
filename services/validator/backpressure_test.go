...
func TestBackpressureGate(t *testing.T) {
	t.Run("mempool tx rejected untouched while overloaded", func(t *testing.T) {
		...
		// The reject must happen before ANY store interaction: no expectations
		// were registered, so any Get/Spend/Create call fails the test here.
...
// depth, including the fail-open behaviour on state errors.
func TestMonitorBlockAssemblyQueue(t *testing.T) {
	...
	// First poll: over the limit. Subsequent polls: state errors, which must
	// keep (not clear) the overloaded verdict.
	baMock.On("GetBlockAssemblyState", mock.Anything).Return(&blockassembly_api.StateMessage{QueueCount: 101}, nil).Once()
	baMock.On("GetBlockAssemblyState", mock.Anything).Return(nil, errors.NewServiceError("state unavailable"))
	...
	// Give the monitor time to process at least one error response: the verdict
	// must not be cleared by a failed poll (fail-open keeps the last one).
	time.Sleep(2 * blockAssemblyQueueMonitorInterval)
	require.True(t, v.blockAssemblyQueueOverloaded.Load(), "a state-poll error must keep the last verdict")
}
