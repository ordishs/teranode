...
// chiR6ShimStore wraps a real store to script the ChiR6 scenario: the combined
...
func (s *chiR6ShimStore) SpendAndCreate(ctx context.Context, tx *bt.Tx, blockHeight uint32, opts ...utxostore.CreateOption) (*meta.Data, []*utxostore.Spend, error) {
	...
	if s.combinedCalls == 1 {
		return nil, nil, errors.NewTxLockedError("chiR6: tx locked on first attempt")
	}

	return nil, nil, errors.NewTxExistsError("chiR6: tx exists in creating state")
}

func (s *chiR6ShimStore) GetMeta(ctx context.Context, hash *chainhash.Hash, data *meta.Data) error {
...
func (s *chiR6ShimStore) FinalizeTransaction(ctx context.Context, tx *bt.Tx) error {
	s.finalizeCalls++
	return s.Store.FinalizeTransaction(ctx, tx)
}

// buildChiR6ValidatorAndTx seeds a real sqlitememory store with a coinbase-style parent,
// wraps it in the ChiR6 shim, and returns a Validator plus a child tx spending the parent.
// It mirrors the below-checkpoint outpoint-only harness (proven to reach SpendAndCreate).
func buildChiR6ValidatorAndTx(t *testing.T, dbName string, rollForwardComplete bool) (*Validator, *bt.Tx, *chiR6ShimStore) {
	t.Helper()
...
// TestValidate_CreateFirst_ErrTxExistsCreating is the ChiR6 regression: when a create-first
// record already exists in the tentative "creating" state (reached here via the TX_LOCKED
// retry loop), the validator must NOT report it as validated. It must roll the record
// forward first, and fail closed if the roll-forward re-spend cannot cover every input.
func TestValidate_CreateFirst_ErrTxExistsCreating(t *testing.T) {
	...
	t.Run("incomplete roll-forward is not treated as validated", func(t *testing.T) {
		v, childTx, shim := buildChiR6ValidatorAndTx(t, "chir6_incomplete", false)

		_, err := v.ValidateWithOptions(ctx, childTx, 500, chiR6Opts())
	...
	t.Run("complete roll-forward finalizes and validates", func(t *testing.T) {
		v, childTx, shim := buildChiR6ValidatorAndTx(t, "chir6_complete", true)

		md, err := v.ValidateWithOptions(ctx, childTx, 500, chiR6Opts())
